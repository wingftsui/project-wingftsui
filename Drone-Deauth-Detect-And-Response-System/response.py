...
def trigger_land(drone_model=None,custom_ip=None):
    ...
    if drone_model is None:
        print("Please provide drone_model in the json setting file.")
        return
    
    profiles=load_drone_json()    
    
    # drone_model is absent in the json setting file.
    if drone_model not in profiles:
        print(f'Cannot find dron_model in json setting file.')
        return
    ...
    try:
        ...
        if profile.get("land_cmd"):
            ...
            print("Json setting file has no land commond")
